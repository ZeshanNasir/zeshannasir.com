...
**Edge Architecture:** Cloudflare Edge + Cloudflare Zero Trust Tunnel (`cloudflared`)  
**Origin Architecture:** Proxmox LXC / Local Homelab + Caddy HTTP/3 Static Engine
...
### 3.1 Cloudflare Ruleset / Single Redirect for `zeshans.dev`
In the Cloudflare dashboard for zone `zeshans.dev`:
1. Navigate to **Rules** → **Redirect Rules** → **Create Rule**.
2. **Rule Name**: `Legacy Migration to zeshannasir.com`
3. **If incoming requests match**:
   - Expression: `(http.host eq "zeshans.dev" or http.host eq "www.zeshans.dev")`
4. **Then**:
   - **Type**: `Dynamic`
...
## 5. Homelab Cloudflare Tunnel (`cloudflared`) Ingress Configuration

Cloudflare Tunnel runs as a lightweight daemon inside your Proxmox cluster (e.g. on `svc-ct-01` or a dedicated lightweight alpine LXC/docker container). It creates outbound-only encrypted QUIC/HTTP2 tunnels to Cloudflare edge points. **No router ports (80/443) are ever opened to the public internet.**
...
ingress:
  ...
  - hostname: zeshannasir.com
    service: http://192.168.20.100:8080
    originRequest:
      connectTimeout: 10s
      noTLSVerify: false
  ...
  # Future Services (Subdomain routing roadmap)
  # - hostname: lab.zeshannasir.com
  #   service: http://192.168.20.24:3000   # Grafana
  # - hostname: status.zeshannasir.com
  #   service: http://192.168.20.11:3001   # Uptime Kuma
...
On the local host or container (`192.168.20.100` or local port `8080`):
```caddyfile
...
- [ ] **Step 5: Configure 301 Edge Redirect**
  - In `zeshans.dev` zone: Create Redirect Rule sending all paths to `https://zeshannasir.com/$1`.
- [ ] **Step 6: Validation**
